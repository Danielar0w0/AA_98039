...
def las_vegas_algorithm(graph, vertices):
    ...
    while len(checked_partitions) < 2 ** len(vertices):
        ...
        subsets = list(combinations(vertices, size))

        # Get random subset
        subset = subsets[np.random.randint(0, len(subsets))]
...
def las_vegas_algorithm_v2(graph, vertices):
    ...
    checked_partitions = []
    while len(checked_partitions) < len(partitions):
